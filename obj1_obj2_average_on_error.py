...
def Test1(ar_coeff, a1, a2, n_obs, H, S, n_rep):
    ...
    while n1 < n_rep:  
        ...
        ############################################################### 
        # A new data generation I want to use.
        # ar_series = np.zeros((num_N, n_obs+S+1))
        # for t in range(1, n_obs+S+1):
        #     ar_series[0][t] = ar_coeff[0] * ar_series[0][t - 1] + noise[0][t-1]
        #     ar_series[1][t] = ar_coeff[1] * ar_series[1][t - 1] + noise[1][t-1]
        #     ar_series[2][t] = ar_coeff[2] * ar_series[2][t - 1] + noise[2][t-1]
        #     ar_series[3][t] = ar_coeff[3] * ar_series[3][t - 1] + noise[3][t-1]
        #     ar_series[4][t] = a1 * ar_series[0][t] + a2 * ar_series[1][t] + noise[4][t-1]
        # for a in range(0, S):
        #     y[a] = ar_series[4][a+1:n_obs+a+1]
        #     X1[a] = np.concatenate([ar_series[i][a+1:n_obs+a+1].reshape(-1, 1) for i in range(P)], axis = 1)
        ################################################################
    ...
    # objective 1: experiment
     ...
     # objective 2: experiment
        for q in range(0, S):
            ...
            # Q3 = np.linalg.inv(np.cov(X.T)) @ V1[q] @ np.linalg.inv(np.cov(X.T)), if we multiply np.linalg.inv(np.cov(X.T)), result for q = 0 should be the same.   
...
#Test
ar_coeff = [0.98, 0.95, 0.98, 0.95];a1=2; a2=3; n_obs = 1000; H = 5; S=10; n_rep = 100
